...
contract MondayToken is Context, IBEP20, Ownable {
    ...
    uint256 private _maxTokenPerAddress = 5 * 10**11 * 10**10;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && from != pancakePair && swapAndLiquifyEnabled) {
            uint256 contractTokenBalance = calculateTaxFee(amount) + calculateLiquidityFee(amount);
            //add liquidity
            swapAndLiquifyAndDistribute(contractTokenBalance);
        }
    ...
    function swapAndLiquifyAndDistribute(uint256 contractTokenBalance) private lockTheSwap {
        // split the contract balance into halves
        uint256 swapBNB = contractTokenBalance * 2 / 3;
        uint256 distribute = (contractTokenBalance - swapBNB) * 3 / 5;
        uint256 liquidity = contractTokenBalance - swapBNB - distribute;
        ...
        // swap tokens for BNB
        swapTokensForBNB(swapBNB); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered.

        distributeToken(distribute);
